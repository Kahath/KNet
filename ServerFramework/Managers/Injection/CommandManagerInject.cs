...
namespace ServerFramework.Managers.Injection
{
	public sealed class CommandManagerInject : ICommandManager, IDisposable
	{
		...
		public void Init()
		{
			...
			Manager.LogMgr.Log(LogTypes.Normal, $"{CommandTable.Count} Commands loaded");
		}
			...
			if (commandPath != null && commandPath.Any())
			{
				...
				if (result != null)
				{
					if (result.Script != null)
					{
						...
						{
							Manager.LogMgr.Log(LogTypes.Error, $"Error with '{result.FullName}' command. wrong arguments", e);
						}
						catch (Exception e)
						{
							Manager.LogMgr.Log(LogTypes.Error, $"Error with '{result.FullName}' command. Failed to execute handler", e);
						}
						...
						if (retVal)
						{
							...
							Manager.DatabaseMgr.AddOrUpdate<ApplicationContext, CommandLogModel>(commandLog);
						}
					}
					else
					{
						Manager.LogMgr.Log(LogTypes.Command, $"Available sub commands for '{result.FullName}'\n{AvailableSubCommands(result, user.UserLevel)}");
					}
				}
				else
				{
					Manager.LogMgr.Log(LogTypes.Command, $"Command '{commandPath[0]}' doesn't exist");
				}
			}
		...
		private void UpdateBase(Command command)
		{
			...
			if (!command.IsValid)
				Manager.LogMgr.Log(LogTypes.Warning, $"Command '{command.FullName}' failed '{command.Validation}' validation");
