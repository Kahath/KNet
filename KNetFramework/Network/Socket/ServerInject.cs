...
namespace KNetFramework.Network.Socket
{
	public class ServerInject : IServer
	{
		...
		private void StartListen()
		{
			...
			Manager.LogManager.Log(LogTypes.Normal, $"Starting listening on {SocketSettings.LocalEndPoint.Address}:{SocketSettings.LocalEndPoint.Port}");
		...
		private void ProcessAccept(SocketAsyncEventArgs e)
		{
			...
			if (e.SocketError == SocketError.Success)
			{
				...
				int id = Manager.SessionManager.AddClient(c);
				...
				if (id != 0)
				{
					...
					try
					{
						Manager.LogManager.Log(LogTypes.Normal, $"Session {socketExtended.ReceiverData.SessionID} ({socketExtended.Receiver.AcceptSocket.RemoteEndPoint}) connected");
					}
					catch (ObjectDisposedException)
					{
		...
		private void ProcessReceive(SocketAsyncEventArgs e)
		{
			if (e.SocketError == SocketError.Success && e.BytesTransferred > 0)
			{
				...
				while (remainingBytes > 0)
				{
					...
					{
						Manager.PacketLogManager.Log(data.Packet);
						Manager.PacketManager.InvokeHandler(data.Packet);
		...
		private void ProcessSend(SocketAsyncEventArgs e)
		{
			if (e.SocketError == SocketError.Success)
			{
				...
				{
					Client c = Manager.SessionManager.GetClient(data.SessionID);
		...
		private SocketExtended CreateNewSendRecPoolItem()
		{
			...
			Manager.BufferManager.SetBuffer(retVal.Receiver);
			Manager.BufferManager.SetBuffer(retVal.Sender);
		...
		private void CloseClientSocket(SocketAsyncEventArgs e)
		{
			...
			Client c = Manager.SessionManager.RemoveClient(data.SessionID);
			data.Reset(data.BufferOffset);
